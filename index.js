        ...
        if (!response) {
            ...
            if (rows.length > 0) {
                ...
                if (formatedData.length > 0 ) {
                    ...
                    if (messageResult1C.length !== 0 && messageResult1C.length > 10) {
            ...
            if (cellValue !== null) {
                ...
                if (indexOfOrder.length === 0) {
                    ...
                    if (rCell && rCell.v !== undefined) {
                        ...
                        return bot.sendMessage(
                            chatId,
                            `${message}Отгружен с фабрики ${DD}/${MM}/${YY}`,
                            { parse_mode: 'HTML' }
                        );
...
const start = async () => {
    ...
    Бот показывает сатус заказа зарубежному поставщику:
    1) оплата по заказу пришла на фабрику
    2) отгружено с фабрики 
    3) вышло со склада перегрузки
    4) поступил на СВХ
    кнопка "Статус заказа поставщику ⌛" в главном меню
    ------------------------------------
    <b>Версия 1.2.0.0
    Что нового:</b>

    Теперь бот при просмотре остатков по 1С
    показывает колличество незаколерованных и не бракованных
    остатков краски на актуальных складах
    ------------------------------------
    <b>Версия 1.1.0.0
    Что нового:</b>
